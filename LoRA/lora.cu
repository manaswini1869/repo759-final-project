...
int main(int argc, char* argv[]) {
 ...
 /*   if (argc != 5) {
    ...
    std::string X_dir = "/srv/home/tsuresh2/repo759/project/repo759-final-project/LoRA/np_ckpts_ece759/Gemma-2-2b/inputs/x_1024.npy";
   ...
   // Allocate device memory
    ...
    checkCudaErrors(cudaMalloc(&d_W, bytes_W));
    ...
    // Step 2: Y = W.X
    cudaEventRecord(start);
    matrixMulKernel<<<dimGrid2, dimBlock>>>(d_X, d_W, d_Y, X_rows, X_cols, B_cols);
    cudaEventRecord(stop);
    cudaEventSynchronize(stop);
    cudaEventElapsedTime(&milliseconds, start, stop);
