...
void random_init(float* matrix, int size) {
    std::mt19937 gen(42); // deterministic for reproducibility
...
void apply_frequency_delta(cufftComplex* freq, int rows, int cols, int C) {
    ...
    std::mt19937 gen(123); // deterministic for reproducibility
    std::uniform_real_distribution<float> dist(-0.1f, 0.1f); // small fine-tuning deltas
    ...
    for (const auto& pos : freq_positions) {
        ...
        freq[idx].x += dist(gen); // add delta real
        freq[idx].y += dist(gen); // add delta imag
    }
}
...
int main(int argc, char* argv[]) {
    ...
    *num_threads = std::stoi(argv[3]);
    if (*rows <= 0 || *cols <= 0 || *num_threads <= 0) {
        std::cerr << "Error: rows, cols, and num_threads must be given & must be positive integers.\n";
        exit(EXIT_FAILURE);
    }
    int size = rows * cols;
    ...
    // Normalize output (cuFFT does not normalize)
