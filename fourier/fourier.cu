    ...
    for (int i = 0; i < r_limit; ++i)
    {
        for (int j = 0; j < c_limit; ++j)
        {
            std::cout << std::fixed << std::setw(18) << std::setprecision(6)
                      << "(" << data[static_cast<size_t>(i) * cols + j].x << "," << data[static_cast<size_t>(i) * cols + j].y << ") ";
        }
...
__global__ void floatToComplexRealOnly(const float *real_in, cufftComplex *complex_out, size_t n)
{
...
int main(int argc, char *argv[])
{
    ...
    size_t ct_mat_bytes = ct_mat_size * sizeof(float); // Size for the float matrix

    // Directories for loading data
    // NOTE: Adjust these paths based on where your executable is relative to the checkpoint files.
    ...
    float *d_ct_mat = nullptr;              // Managed memory for the reconstructed dense float matrix (Input to IFFT Prep)
    cufftComplex *d_freq_complex = nullptr; // Managed memory for the complex freq data (Input to IFFT)
    float *d_spatial_real = nullptr;        // Managed memory for the spatial data (Output of IFFT), i.e., delta(w)
    ...
    try
    {
        ...
        std::cout << "Loading coefficients from " << ct_directory << " into host memory..." << std::endl;
        auto ct_dims = load_ckpt_host_float(ct_directory, h_ct); // h_ct allocated by function
        ct_rows_loaded = std::get(ct_dims);
        ct_cols_loaded = std::get(ct_dims);
        if (h_ct == nullptr || ct_cols_loaded <= 0)
        {
            throw std::runtime_error("Failed to load coefficients or zero coefficients loaded.");
        }
        ct_sparse_size = static_cast<size_t>(ct_rows_loaded) * ct_cols_loaded;
        size_t ct_sparse_bytes = ct_sparse_size * sizeof(float);
        std::cout << "Loaded " << ct_sparse_size << " sparse coefficients (" << ct_sparse_bytes << " bytes). Shape (" << ct_rows_loaded << "x" << ct_cols_loaded << ")" << std::endl;

        // 2. Allocate Managed Memory for Sparse Coefficients and Copy
        std::cout << "Allocating CUDA managed memory for sparse coefficients..." << std::endl;
        cudaMallocManaged(&d_ct, ct_sparse_bytes);
        std::cout << "Copying sparse coefficients from host to managed memory..." << std::endl;
        memcpy(d_ct, h_ct, ct_sparse_bytes); // Host-side copy is fine for managed memory

        // 3. Free Temporary Host Memory for Sparse Coefficients
        std::cout << "Freeing temporary host memory for sparse coefficients..." << std::endl;
        delete[] h_ct;
        h_ct = nullptr;

        std::cout << "\n--- First few loaded sparse coefficients (d_ct array, managed memory) ---" << std::endl;
        ...
        auto locs_dims = load_ckpt_host_int(locs_directory, h_locs); // h_locs allocated by function
        locs_rows_loaded = std::get(locs_dims);
        locs_cols_loaded = std::get(locs_dims);
        if (h_locs == nullptr || (locs_rows_loaded <= 0 && locs_cols_loaded <= 0))
        { // Handle case where file exists but is empty/malformed
            throw std::runtime_error("Failed to load locations or zero locations loaded.");
        }
        ...
        if (ct_sparse_size != locs_cols_loaded)
        { // Expect locs_cols_loaded to match the number of coefficients
            std::cerr << "Warning: Mismatch! Number of coefficients (" << ct_sparse_size
                      << ") != number of location columns (" << locs_cols_loaded << "). Using minimum for reconstruction." << std::endl;
        }
        if (locs_rows_loaded != 2)
        {
            std::cerr << "Warning: Expected locations array to have 2 rows (row, col indices), but got " << locs_rows_loaded << ". Assuming format [row_indices..., col_indices...]." << std::endl;
        }
        if (locs_sparse_size != 2 * locs_cols_loaded)
        {
            std::cerr << "Warning: Expected total locations elements to be 2 * num_coefficients, but got " << locs_sparse_size << " vs " << 2 * locs_cols_loaded << ". Check locs file format." << std::endl;
        }

        std::cout << "Loaded " << locs_sparse_size << " location indices (" << locs_sparse_bytes << " bytes). Shape (" << locs_rows_loaded << "x" << locs_cols_loaded << ")" << std::endl;

        // 5. Allocate Managed Memory for Sparse Locations and Copy
        std::cout << "Allocating CUDA managed memory for sparse locations..." << std::endl;
        cudaMallocManaged(&d_locs, locs_sparse_bytes);
        std::cout << "Copying sparse locations from host to managed memory..." << std::endl;
        ...
        std::cout << "Initializing dense matrix d_ct_mat to zero using cudaMemset..." << std::endl;
        cudaMemset(d_ct_mat, 0, ct_mat_bytes);

        // 9. Reconstruct Dense Matrix (on Host, accessing Managed Memory)
        // Note: This reconstruction happens on the host using managed memory pointers.
        // For very large matrices, a kernel could be more efficient.
        std::cout << "Reconstructing dense matrix d_ct_mat on host using managed memory pointers..." << std::endl;
        size_t num_elements_to_copy = std::min(ct_sparse_size, static_cast<size_t>(locs_cols_loaded)); // Use min of coefficients loaded and location columns
        ...
        for (size_t i = 0; i < num_elements_to_copy; ++i)
        {
            ...
            {
                row_idx = d_locs[i];
                col_idx = d_locs[locs_cols_loaded + i]; // Assumes [rows..., cols...] structure
            }
            else
            {
                ...
                if (i == 0)
                    std::cerr << "Error: Unexpected location data format or insufficient elements in locs array for coefficient " << i << ". Expected at least " << 2 * locs_cols_loaded << " elements for shape (2," << locs_cols_loaded << ")." << std::endl;
            ...
            {
                if (i < 10)
                { // Print only a few warnings to avoid flooding console
                    std::cerr << "Warning: Out-of-bounds location index (row=" << row_idx << ", col=" << col_idx
                              << ") for coefficient index " << i << ". Skipping." << std::endl;
                }
                else if (i == 10)
                {
                    std::cerr << "Warning: Further out-of-bounds warnings suppressed." << std::endl;
                }
            }
        }
        ...
        std::cout << "\nAllocating CUDA managed memory for complex frequency input (d_freq_complex)..." << std::endl;
        cudaMallocManaged(&d_freq_complex, freq_complex_bytes);

        // 11. Copy Reconstructed Float Matrix to Complex Matrix (Imaginary=0) using Kernel
        std::cout << "Copying dense float matrix to complex frequency matrix (imaginary=0) using kernel..." << std::endl;
        int threadsPerBlock = 256;
        int numBlocks = (freq_complex_size + threadsPerBlock - 1) / threadsPerBlock;
        floatToComplexRealOnly<<<numBlocks, threadsPerBlock>>>(d_ct_mat, d_freq_complex, freq_complex_size);
        ...
        {
            std::cout << "\n--- Complex Frequency Matrix (d_freq_complex, managed memory) ---" << std::endl;
            print_complex_matrix(d_freq_complex, ct_mat_rows, ct_mat_cols, ct_mat_rows, ct_mat_cols, "Complex Frequency Matrix");
            std::cout << "------------------------------------------------------------------\n"
                      << std::endl;
        }
        else
        {
            std::cout << "(Skipping print of large complex frequency matrix d_freq_complex)" << std::endl;
        }

        // 12. Allocate Managed Memory for Real Spatial Output (delta(w))
        size_t spatial_real_size = static_cast<size_t>(ct_mat_rows) * ct_mat_cols; // C2R IFFT of MxN complex gives MxN real
        ...
        int dims[] = {ct_mat_cols, ct_mat_rows}; // cuFFT dimensions are {nx, ny} = {cols, rows}
        cufftPlanMany(&plan,
                      2,          // rank (2D)
                      dims,       // dimensions {nx, ny}
                      NULL, 1, 0, // Input layout (contiguous complex, istride=1, idist=0 for batch=1)
                      NULL, 1, 0, // Output layout (contiguous real, ostride=1, odist=0 for batch=1)
        ...
        std::cout << "Applying scaling factor (" << scale_factor << ") to the output..." << std::endl;
        numBlocks = (spatial_real_size + threadsPerBlock - 1) / threadsPerBlock; // Recalculate if needed, but size is the same as complex input size
        normalizeReal<<<numBlocks, threadsPerBlock>>>(d_spatial_real, spatial_real_size, scale_factor);
        cudaGetLastError();      // Check for kernel launch errors
        cudaDeviceSynchronize(); // Wait for kernel to finish
    ...
    std::cout << "Execution finished successfully. Delta(w) computed." << std::endl;
    return 0;
}